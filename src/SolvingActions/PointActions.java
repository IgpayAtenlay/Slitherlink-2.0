...
public class PointActions {
    ...
    public static void fillDiagonals(Memory memory, Coords coords) {
        for (DiagonalDirection direction : DiagonalDirection.values()) {
            ...
            if (lineOne != Line.EMPTY && lineTwo != Line.EMPTY) {
                if (lineOne == lineTwo) {
                    memory.setDiagonal(false, Diagonal.BOTH_OR_NEITHER, coords, direction, false);
                } else if (lineOne.getOpposite() == lineTwo) {
                    memory.setDiagonal(false, Diagonal.EXACTLY_ONE, coords, direction, false);
                }
            } else if (lineOne == Line.LINE || lineTwo == Line.LINE) {
                memory.setDiagonal(false, Diagonal.AT_LEAST_ONE, coords, direction, false);
            } else if (lineOne == Line.X || lineTwo == Line.X) {
                memory.setDiagonal(false, Diagonal.AT_MOST_ONE, coords, direction, false);
            }
        }
    }
    ...
    public static void copyDiagonals(Memory memory, Coords coords) {
        for (DiagonalDirection direction : DiagonalDirection.values()) {
            Diagonal diagonal = memory.getDiagonal(false, coords, direction);
            switch (diagonal) {
                case BOTH_OR_NEITHER, EXACTLY_ONE -> memory.setDiagonal(false, diagonal, coords, direction.getOpposite(), false);
                case AT_LEAST_ONE -> memory.setDiagonal(false, Diagonal.AT_MOST_ONE, coords, direction.getOpposite(), false);
            }
